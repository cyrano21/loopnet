...
export default function FavoritesPage() {
  ...
  const removeSelected = async () => {
    try {
      await Promise.all(selectedItems.map((id) => fetch(`/api/favorites/${id}`, { method: "DELETE" })))
      setFavorites(favorites.filter((fav: any) => !selectedItems.includes(fav.id)))
      setSelectedItems([])
    } catch (error) {
    ...
    <Card className="overflow-hidden hover:shadow-lg transition-shadow">
      <div className="relative">
        ...
        <div className="absolute top-2 right-2 flex gap-2">
          <Badge className={property.status === "For Sale" ? "bg-green-600" : "bg-blue-600"}>{property.status}</Badge>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {favorites.length} propriété{favorites.length !== 1 ? "s" : ""} sauvegardée
        ...
        {favorites.length === 0 ? (
          <Card className="p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">
              Commencez à sauvegarder des propriétés qui vous intéressent pour les retrouver facilement ici.
            </p>
            <Button asChild>
            ...
            {selectedItems.length > 0 && (
              <Card className="p-4 mb-6 bg-blue-50 border-blue-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <Checkbox checked={selectedItems.length === favorites.length} onCheckedChange={toggleSelectAll} />
