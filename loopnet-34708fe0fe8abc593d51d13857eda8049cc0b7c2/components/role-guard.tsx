...
export function RoleGuard({ children, allowedRoles = [], requiredRole, fallback, redirectTo }: RoleGuardProps) {
      ...
      <div className="container mx-auto px-4 py-8">
        <Card className="max-w-md mx-auto">
          <CardHeader className="text-center">
            ...
            <CardDescription>{getRoleMessage(targetRole, userRole)}</CardDescription>
          </CardHeader>
          <CardContent className="text-center space-y-4">
            {targetRole !== "admin" && (
              <Button onClick={() => router.push(`/pricing?upgrade=${targetRole}`)} className="w-full">
...
function checkRoleAccess(userRole: string, allowedRoles: string[], requiredRole?: string): boolean {
