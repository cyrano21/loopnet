...
import { Building2, ArrowRight, ArrowLeft, Check, Upload, MapPin } from "lucide-react"
...
import { getPropertyTypeByValue, getTransactionTypeByValue } from "@/lib/property-types"
...
export default function ListPropertyPage() {
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-6">
            <div>
              <h2 className="text-2xl font-bold mb-4">Quel type de bien souhaitez-vous publier ?</h2>
              <PropertyTypeSelector
                selectedType={formData.propertyType}
                selectedTransaction={formData.transactionType}
                onTypeChange={(type) => setFormData({ ...formData, propertyType: type })}
                onTransactionChange={(transaction) => setFormData({ ...formData, transactionType: transaction })}
              />
            </div>
          </div>
        )
      ...
      case 2:
        return (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold mb-4">Informations sur votre bien</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="price">Prix * ({formData.transactionType === "sale" ? "€" : "€/mois"})</Label>
                    <Input
                      id="price"
                      type="number"
                      placeholder="450000"
                ...
                {(formData.propertyType === "office" || formData.propertyType === "apartment") && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-100 rounded-lg p-6 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-500">Positionnez votre bien</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                  ...
                  <p className="text-gray-600 mb-4">Glissez-déposez ou cliquez pour sélectionner</p>
                  <Button variant="outline" disabled={uploading}>
                    {uploading ? `Upload... ${progress}%` : "Choisir des photos"}
                  </Button>
                  <input
                    type="file"
                    multiple
                ...
                {formData.images.length > 0 && (
                  <div className="mt-4">
                    <h4 className="font-semibold mb-2">Photos ajoutées ({formData.images.length})</h4>
              ...
              <div>
                <h3 className="font-semibold mb-4">Conseils pour de bonnes photos</h3>
                <div className="space-y-3 text-sm">
                  <div className="flex items-start gap-2">
                    <Check className="h-4 w-4 text-green-600 mt-0.5" />
                    <span>Prenez des photos avec une bonne luminosité naturelle</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <Check className="h-4 w-4 text-green-600 mt-0.5" />
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setFormData({ ...formData, visitSchedule: e.target.value })}
                  />
                </div>
                ...
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800">
                    Vos informations sont protégées et ne seront partagées qu'avec les personnes intéressées.
                  </p>
                </div>
              </div>
            </div>
      ...
      case 6:
        return (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold mb-4">Récapitulatif et publication</h2>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm">
                        {getTransactionTypeByValue(formData.transactionType)?.label}
                      </span>
                    ...
                    <div className="text-2xl font-bold text-blue-600">
                      {Number(formData.price).toLocaleString("fr-FR")}€{formData.transactionType === "rent" && "/mois"}
                    </div>

                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-600">Surface</span>
                        <div className="font-semibold">{formData.surface} m²</div>
                      </div>
                      {formData.rooms && (
                        <div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="p-4 bg-green-50 rounded-lg">
                      <h4 className="font-medium text-green-900 mb-2">✅ Publication gratuite</h4>
                      <p className="text-sm text-green-800">Votre annonce sera visible pendant 30 jours</p>
                    </div>

                    <div className="p-4 bg-yellow-50 rounded-lg">
                      <h4 className="font-medium text-yellow-900 mb-2">⏳ Validation</h4>
                      <p className="text-sm text-yellow-800">Votre annonce sera vérifiée sous 24h avant publication</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 2:
        return formData.title && formData.price && formData.surface && formData.description
      case 3:
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-8">
              ...
              <nav className="hidden md:flex space-x-6">
                <Link href="/list-property" className="text-blue-600 font-medium">
                  Publier une annonce
                </Link>
                <Link href="/my-properties" className="text-gray-700 hover:text-blue-600">
                  Mes annonces
                </Link>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between">
          ...
          {currentStep < steps.length ? (
            <Button onClick={() => setCurrentStep(Math.min(steps.length, currentStep + 1))} disabled={!canProceed()}>
              Suivant
