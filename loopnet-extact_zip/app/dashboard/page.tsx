              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{analytics?.monthlyRevenue?.toLocaleString("fr-FR") || 0}€</p>
                    <div className="flex items-center gap-1 text-sm text-green-600">
                      <ArrowUp className="w-4 h-4" />+{analytics?.monthlyGrowth || 0}% ce mois
                    </div>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <Progress value={analytics?.occupancyRate || 0} className="mt-2" />
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{analytics?.recentInquiries || 0}</p>
                    <p className="text-xs text-gray-500">Sur {analytics?.totalInquiries || 0} total</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{analytics?.activeProperties || 0}</p>
                    <p className="text-xs text-orange-600">{analytics?.pendingProperties || 0} en attente</p>
                  </div>
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Badge className="bg-green-100 text-green-800">
                  ${analytics?.monthlyRevenue?.toLocaleString() || 0} / ${analytics?.monthlyGoal?.toLocaleString() || 0}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Progress: {analytics?.goalProgress || 0}%</span>
                  <span>
                    ${((analytics?.monthlyGoal || 0) - (analytics?.monthlyRevenue || 0)).toLocaleString()} remaining
                  </span>
                </div>
                <Progress value={analytics?.goalProgress || 0} className="h-3" />
              </div>
            </CardContent>
          </Card>
                      ...
                      analytics?.recentActivities?.map((activity) => (
                        <div key={activity.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500">{new Date(activity.time).toLocaleString("fr-FR")}</p>
                          </div>
                  ...
                  <CardContent className="space-y-3">
                    {analytics?.topProperties?.length === 0 ? (
                      ...
                      analytics?.topProperties?.slice(0, 3).map((property, index) => (
                        <div key={property._id} className="flex items-center gap-3 p-2 rounded border">
                          <div className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 text-blue-600 font-bold text-sm">
                            {index + 1}
                          </div>
                          <div className="flex-1">
                            <p className="font-medium text-sm">{property.title}</p>
                            <p className="text-xs text-gray-500">{property.views} vues</p>
                          </div>
                        </div>
                      ))
                    )}
                ...
                <CardContent>
                  {properties.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600 mb-6">Commencez par ajouter votre première propriété</p>
                      <Link href="/list-property">
                        <Button>
                  ...
                  <CardContent>
                    <div className="h-64 bg-gradient-to-br from-green-50 to-emerald-100 rounded-lg flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-500">Données interactives</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold mb-2">Système de tâches</h3>
                    <p className="text-gray-600">Planification et suivi des activités</p>
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">Génération de rapports PDF et Excel</p>
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
...
export default function DashboardPage() {
  return (
    <RoleGuard allowedRoles={["admin", "agent", "premium", "simple"]} redirectTo="/auth/signin">
      <DashboardContent />
    </RoleGuard>
  )
}
