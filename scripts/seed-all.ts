...
const seedOrder = [
  { name: 'Users', fn: seedUsers, description: 'Création des utilisateurs de base' },
  { name: 'Properties', fn: seedProperties, description: 'Création des propriétés' },
  { name: 'Professionals', fn: seedProfessionals, description: 'Création des profils professionnels' },
  { name: 'News', fn: seedNews, description: 'Création des articles de presse' },
  { name: 'Inquiries', fn: seedInquiries, description: 'Création des demandes d\'information' },
  { name: 'SavedSearches', fn: seedSavedSearches, description: 'Création des recherches sauvegardées' },
  { name: 'SearchAlerts', fn: seedSearchAlerts, description: 'Création des alertes de recherche' },
  { name: 'Favorites', fn: seedFavorites, description: 'Création des favoris' },
  { name: 'Commissions', fn: seedCommissions, description: 'Création des commissions' },
  { name: 'Tasks', fn: seedTasks, description: 'Création des tâches' },
  { name: 'Reports', fn: seedReports, description: 'Création des rapports' },
  { name: 'MarketAnalysis', fn: seedMarketAnalysis, description: 'Création des analyses de marché' },
  { name: 'PriceHistory', fn: seedPriceHistory, description: 'Création de l\'historique des prix' },
  { name: 'Notifications', fn: seedNotifications, description: 'Création des notifications' },
  { name: 'Advertisements', fn: seedAdvertisements, description: 'Création des publicités' }
]
...
function formatResults(results: SeedResult[]): void {
  ...
  if (successful.length > 0) {
    ...
    successful.forEach(result => {
      console.log(`   ✓ ${result.name.padEnd(20)} - ${formatDuration(result.duration)}`)
    })
  }
  ...
  try {
    ...
    if (only && only.length > 0) {
      seedsToRun = seedOrder.filter(seed => 
        only.some(name => seed.name.toLowerCase().includes(name.toLowerCase()))
      )
      console.log(`🎯 Exécution sélective: ${seedsToRun.map(s => s.name).join(', ')}`)
    }
    
    if (exclude && exclude.length > 0) {
      seedsToRun = seedsToRun.filter(seed => 
        !exclude.some(name => seed.name.toLowerCase().includes(name.toLowerCase()))
      )
    ...
    for (let i = 0; i < seedsToRun.length; i++) {
      ...
      try {
        console.log(`[${i + 1}/${seedsToRun.length}] 🔄 ${seed.name}: ${seed.description}`)
        ...
        console.error(`   ❌ Échec après ${formatDuration(duration)}: ${errorMessage}\n`)
...
export async function showDatabaseStats() {
  try {
    ...
    for (const collection of collections) {
      ...
      console.log(`${collection.name.padEnd(20)}: ${count.toLocaleString('fr-FR')} documents`)
    }
    ...
    console.log(`Taille de la base: ${(stats.dataSize / 1024 / 1024).toFixed(2)} MB`)
